...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if q_len > 1:
            tmp_q_state = query_states[:,:,-1,:].clone()
            tmp_q_state = tmp_q_state.unsqueeze(2)
            tmp_k_states = key_states.clone()
            attn_scores = torch.matmul(tmp_q_state, tmp_k_states.transpose(-2, -1))  # → [1, 32, 1, 623]
            # 可选：进行 scale（常用于 Transformer）
            d_k = tmp_q_state.size(-1)  # 通常是 128
            attn_scores = attn_scores / (d_k ** 0.5)  # scaled dot-product
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def get_div_initial_indices(self,H,W,k):
        ...
        rows = indices // W
        cols = indices % W
        row_diff = torch.abs(rows[:, None] - rows[None, :])
        col_diff = torch.abs(cols[:, None] - cols[None, :])
        manhattan_distance = row_diff + col_diff
        manhattan_distance = manhattan_distance.float()
        manhattan_distance.fill_diagonal_(float('inf'))
        select_indices = [0]
        manhattan_distance[:,select_indices] = float('-inf')
        num = len(select_indices)
        while num < k:
            if len(select_indices) == 0:
                row_min_values, row_min_indices = manhattan_distance.min(dim=1)
                max_value, max_index = row_min_values.max(dim=0)
                select_indices.append(max_index)
                manhattan_distance[:,max_index] = float('-inf')
            else:
                selected_dis = manhattan_distance[select_indices,:]
    ...
    def attn_prune(self,image_attn,select_num,extra_num,k):
        # print("select",select_num)
        extra_select_num = extra_num
        mean_attn = torch.mean(image_attn, dim=1)
        top_attn_values, attn_indices = torch.topk(mean_attn, extra_select_num, dim=-1)
        pre_indices = attn_indices[attn_indices < k].clone()
        post_indices = attn_indices[attn_indices >= k].clone()
        pre_num = pre_indices.shape[0]
        # print("pre num",pre_num)
        post_num = select_num - pre_indices.shape[0]
        if post_num > 0:
            post_indices = post_indices[:post_num]
            attn_indices = torch.cat((pre_indices, post_indices), dim=0)
        else:
    ...
    def forward(
        ...
        layer_index = 0
        prelayer_attention = None
        
        for decoder_layer in self.layers:
            layer_index += 1
            if self.use_flash_pruning:
                if input_ids is None:
                    if layer_index == self.start_layer:
                        ...
                        remain_sys_index = torch.tensor(list(range(35)))
                        remain_text_index = torch.tensor(list(range(611,total_length)))
                        all_remained_index = torch.cat((remain_sys_index, remain_img_index, remain_text_index)).to(hidden_states.device)
                        hidden_states = hidden_states[:,all_remained_index,:]
                        position_ids = all_remained_index.clone().to(dtype=position_ids.dtype,device=position_ids.device)
                        position_ids = position_ids.unsqueeze(0)
                    elif layer_index == self.img_sense_layer:
                        total_length = hidden_states.shape[1]
                        ...
                        hidden_states = hidden_states[:,all_remained_index,:]
                        self.hidden_length = k
                        position_ids = position_ids[:,all_remained_index]
                    elif layer_index == self.rel_start_layer:
                        total_length = hidden_states.shape[1]
                        ...
                        self.hidden_length = k
                        position_ids = position_ids[:,all_remained_index]
                    elif layer_index == (self.end_layer+1):
                        total_length = hidden_states.shape[1]
                        remain_sys_index = torch.tensor(list(range(35)))
                        txt_start = 35 + self.hidden_length
                        remain_txt_index = torch.tensor(list(range(txt_start,total_length)))
                        all_remained_index = torch.cat((remain_sys_index, remain_txt_index)).to(hidden_states.device)
                        hidden_states = hidden_states[:,all_remained_index,:]
                        position_ids = position_ids[:,all_remained_index]
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs,_ = self._gradient_checkpointing_func(
                    ...
                    layer_outputs,prelayer_attention = decoder_layer(
                        hidden_states,
