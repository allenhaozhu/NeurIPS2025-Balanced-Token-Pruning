...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        # Calculate target attention
        target_q_len = query_states.shape[1]
        if target_q_len > 1:
            target_q = query_states[:,-1,:,:].clone()
            target_q = target_q.unsqueeze(1)
            target_k = key_states.clone()
            targetq_ = target_q.permute(0, 2, 1, 3)        # [1, 28, 1, 128]
            targetk_ = target_k.permute(0, 2, 3, 1)        # [1, 28, 128, 142]       
            attn_scores = torch.matmul(targetq_, targetk_)
            attn_scores = torch.softmax(attn_scores / (128 ** 0.5), dim=-1)
        if (
        ...
        for decoder_layer in self.layers:
            if self.use_flash_pruning:
                ...
                if hidden_states.shape[1] > 1:
                    if self.our_method:
                        if layer_index == self.start_layer:
                            ...
                            # # Divprune
                            # div_select_num = int(k*0.8)
                            # pre_hidden_state = hidden_states.squeeze(0).clone()
                            # img_hidden_state = pre_hidden_state[self.img_start_idx:self.img_start_idx+self.img_num]
                            # div_indices = self.div_prune(img_hidden_state.clone(),div_select_num,None)
                            # # print(div_indices)
                            # # Atten prune
                            # atten_select_num = int(k * 0.2)
                            # last_token_attn = pre_layer_atten[:,:,-1,:]
                            # image_attention = last_token_attn[:,:,self.img_start_idx:self.img_start_idx+self.img_num]
                            # attn_indices = self.attn_prune(image_attention,atten_select_num)
                            # # Merge two selection
                            # union_indices = list(set(attn_indices.cpu().tolist()) | set(div_indices.tolist()))
                            # # print(union_indices)
                            # now_num = len(union_indices)
                            # left_num = k-now_num
                            # if left_num > 0:
                            #     indices = self.div_prune(img_hidden_state.clone(),left_num,union_indices)
                            #     # print(union_indices)
                            #     # print(indices)
                            # else:
                            #     if left_num < 0:
                            ...
                            '''换成另一种'''
                            atten_select_num = int(k * 0.1)
                            extend_select_num = int(atten_select_num * 1.2)
                            last_token_attn = pre_layer_atten[:,:,-1,:]
                            image_attention = last_token_attn[:,:,self.img_start_idx:self.img_start_idx+self.img_num]
                            mean_attn = torch.mean(image_attention, dim=1)
                            top_attn_values, attn_indices = torch.topk(mean_attn, extend_select_num, dim=-1)
                            attn_indices = attn_indices.squeeze().tolist()
                            # # #基于attention初始化的集合进行diversity筛选
                            true_atten_num = len(attn_indices)
                            div_select_num = k-true_atten_num
                            if div_select_num > 0:
                                pre_hidden_state = hidden_states.squeeze(0).clone()
                                img_hidden_state = pre_hidden_state[self.img_start_idx:self.img_start_idx+self.img_num]
                                indices = self.div_prune(img_hidden_state,div_select_num,attn_indices)
                            else:
                                indices = torch.tensor(attn_indices)
                            '''换成另一种'''
                            indices = indices.squeeze().cpu()
                            indices, _ = torch.sort(indices)
                            remain_img_index = indices + self.img_start_idx.cpu()
                            remain_sys_index = torch.tensor(list(range(self.img_start_idx)))
                            remain_text_index = torch.tensor(list(range(self.img_start_idx+self.img_num,total_length)))
                            ...
                            '''这里position ids 被改成了原始的位置index'''
                            first_pos_item = position_embeddings[0].clone()
                            sec_pos_item = position_embeddings[1].clone()
                            first_pos_item = first_pos_item[:,:,all_remained_index,:]
                            sec_pos_item = sec_pos_item[:,:,all_remained_index,:]
                            position_embeddings = (first_pos_item,sec_pos_item)
                        elif layer_index == self.img_sense_layer:
                            total_length = hidden_states.shape[1]
                            k = self.hidden_length // 2
                            img_end = self.img_start_idx + self.hidden_length
                            pre_hidden_state = hidden_states.squeeze(0).clone()
                            img_hidden_state = pre_hidden_state[self.img_start_idx:img_end]
                            atten_select_num = int(k * 0.4)
                            last_token_attn = pre_layer_atten[:,:,-1,:]
                            image_attention = last_token_attn[:,:,self.img_start_idx:img_end]
                            attn_indices = self.attn_prune(image_attention,atten_select_num)
                            union_list = attn_indices.cpu().tolist()
                            now_num = len(union_list)
                            left_num = k-atten_select_num
                            if left_num > 0:
                                # print(len(union_list))
                                indices = self.div_prune(img_hidden_state.clone(),left_num,union_list)
                                # print(indices.shape[0])
                            else:
                                if left_num < 0:
                                    print("Bug!")
                                indices = torch.tensor(union_list)
                            indices = indices.squeeze().cpu()
                            indices, _ = torch.sort(indices)
                            remain_img_index = indices + self.img_start_idx.cpu()
                            remain_sys_index = torch.tensor(list(range(self.img_start_idx)))
                            ...
                            last_token_attn = pre_layer_atten[:,:,-1,:]
                            image_attention = last_token_attn[:,:,self.img_start_idx:img_end]
                            # indices = self.attn_prune(image_attention,atten_select_num)
                            extra_atten_num = int(k * 1.2)
                            # '''这里消融'''
                            mean_attn = torch.mean(image_attention, dim=1)
                            top_attn_values, indices = torch.topk(mean_attn, extra_atten_num, dim=-1)
                            indices = indices.squeeze()
                            pre_indices = indices[indices < k].clone()
                            post_indices = indices[indices >= k].clone()
                            post_num = k - pre_indices.shape[0]
                            if post_num > 0:
                                post_indices = post_indices[:post_num]
                                indices = torch.cat((pre_indices, post_indices), dim=0)
                            else:
                                indices = pre_indices
                            ...
                            sec_pos_item = sec_pos_item[:,:,all_remained_index,:]
                            position_embeddings = (first_pos_item,sec_pos_item)
                        elif layer_index == self.end_layer:
                            total_length = hidden_states.shape[1]
                            img_end = self.img_start_idx + self.hidden_length
                            ...
                            #     image_attention = last_token_attn[:,:,self.img_start_idx:img_end]
                            #     mean_attn = torch.mean(image_attention, dim=1)
                            #     top_attn_values, indices = torch.topk(mean_attn, k, dim=-1)
                            ...
                            remain_sys_index = torch.tensor(list(range(self.img_start_idx)))
                            remain_text_index = torch.tensor(list(range(img_end,total_length)))
                            all_remained_index = torch.cat((remain_sys_index, remain_text_index)).to(hidden_states.device)
                            hidden_states = hidden_states[:,all_remained_index,:]
                            #构造attn_mask和position ids 这里需要看下attention mask和之前一样不
                            first_pos_item = position_embeddings[0].clone()
                            sec_pos_item = position_embeddings[1].clone()
                            first_pos_item = first_pos_item[:,:,all_remained_index,:]
                            sec_pos_item = sec_pos_item[:,:,all_remained_index,:]
                        ...
                        if layer_index == self.vtw_layer:
                            # print(self.img_start_idx)
                            # print(self.img_num)
                            # print("pre_layer",pre_layer_atten.shape)
                            total_length = hidden_states.shape[1]
                            remain_sys_index = torch.tensor(list(range(self.img_start_idx)))
                            remain_text_index = torch.tensor(list(range(self.img_start_idx+self.img_num,total_length)))
                            all_remained_index = torch.cat((remain_sys_index, remain_text_index)).to(hidden_states.device)
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs, pre_layer_atten = decoder_layer(
                        hidden_states,
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                indices = mask.squeeze()
                indices = torch.where(indices)[0][0]
                # print('mask shape is',mask.shape)
                self.model.img_start_idx = indices
                mask_unsqueezed = mask.unsqueeze(-1)
                mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
                image_mask = mask_expanded.to(inputs_embeds.device)

                image_embeds = image_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                self.model.img_num = image_embeds.shape[0]
